    ...
    G: GroupedValue,
{
    ...
    pub fn from(values: impl IntoIterator<Item = (K, G::Item)>) -> Group<K, G> {
...
impl Shapes {
    ...
    pub fn to_summary(&self) -> String {
        if self.shapes.len() == 1 {
            let (key, shapes) = self.shapes.iter().nth(0).unwrap();

            format!("[table: {} rows as {}]", shapes.len(), key.describe_str())
        } else {
            let size: usize = self.shapes.values().map(|v| v.len()).sum();

            format!("[table: {} rows]", size)
        }
    }
